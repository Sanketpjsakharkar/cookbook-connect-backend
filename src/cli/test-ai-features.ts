...
async function testAIFeatures() {
  ...
  try {
    ...
    logger.log(
      `OpenAI Service Health: ${isHealthy ? '✅ Healthy' : '❌ Unhealthy'}`,
    );
    ...
    try {
      ...
      logger.log(
        `Error handling result: ${invalidResponse ? 'Unexpected success' : 'Graceful failure ✅'}`,
      );
    } catch (error) {
    ...
    const successCount = perfResults.filter(
    ...
    logger.log(
      `- Performance: ${perfTime < 15000 && successCount >= 2 ? '✅' : '⚠️'}`,
    );
