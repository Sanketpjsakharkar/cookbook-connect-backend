...
export class RecipesService {
  ...
  async update(id: string, userId: string, updateRecipeInput: UpdateRecipeInput) {
  ...
  async findByIngredients(ingredientNames: string[], limit: number = 20) {
    const recipes = await this.recipesRepository.findByIngredients(ingredientNames, limit);
    return recipes.map(recipe => this.mapRecipeWithStats(recipe));
  }

  async getUserFeed(userId: string, skip?: number, take?: number) {
    const recipes = await this.recipesRepository.findUserFeed(userId, skip, take);
    return recipes.map(recipe => this.mapRecipeWithStats(recipe));
  }
