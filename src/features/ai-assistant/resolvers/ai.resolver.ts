...
export class AIResolver {
  ...
  async analyzeRecipe(
    ...
    try {
      ...
      const analysis = await this.recipeAnalyzerService.analyzeRecipe(
        input.recipeId,
        _user.id,
      );
      ...
      this.logger.debug(
        `Recipe analysis completed in ${responseTime}ms for recipe ${input.recipeId}`,
      );
      return analysis;
    } catch (error) {
  ...
  async getQuickSuggestions(
    ...
    try {
      return await this.recipeAnalyzerService.getQuickSuggestions(
        input.recipeId,
        _user.id,
      );
    } catch (error) {
      this.logger.error(
        `Failed to get quick suggestions for recipe ${input.recipeId}:`,
        error,
      );
      return null;
    }
  ...
  async getRecipeComplexity(
    ...
    try {
      const complexity =
        await this.recipeAnalyzerService.analyzeRecipeComplexity(recipeId);
      return complexity ? JSON.stringify(complexity) : null;
    } catch (error) {
      this.logger.error(
        `Failed to analyze recipe complexity for ${recipeId}:`,
        error,
      );
      return null;
    }
  ...
  async getIngredientSubstitutions(
    ...
    try {
      ...
      this.logger.debug(
        `Substitutions generated in ${responseTime}ms for recipe ${input.recipeId}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `Failed to get substitutions for recipe ${input.recipeId}:`,
        error,
      );
      return null;
    }
  ...
  async getSingleIngredientSubstitution(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get single ingredient substitution for ${input.ingredient}:`,
        error,
      );
      return null;
    }
  ...
  async getDietarySubstitutions(
    @Args('recipeId') recipeId: string,
    @Args('dietaryNeed', { type: () => DietaryRestriction })
    ...
    try {
      ...
      this.logger.error(
        `Failed to get dietary substitutions for recipe ${recipeId}:`,
        error,
      );
      return null;
    }
  ...
  async getCommonSubstitutions(): Promise<string | null> {
    try {
      const substitutions =
        await this.ingredientSubstitutionService.getCommonSubstitutions();
  ...
  async getCookingTips(
    ...
    try {
      const startTime = Date.now();
      const guidance = await this.cookingTipsService.getCookingTips(
        input.recipeId,
        _user.id,
      );
      const responseTime = Date.now() - startTime;
      ...
      this.logger.debug(
        `Cooking tips generated in ${responseTime}ms for recipe ${input.recipeId}`,
      );
      return guidance as any; // Type conversion for enum compatibility
    } catch (error) {
      this.logger.error(
        `Failed to get cooking tips for recipe ${input.recipeId}:`,
        error,
      );
      return null;
    }
  ...
  async getTroubleshootingAdvice(
    ...
    try {
      return await this.cookingTipsService.getTroubleshootingAdvice(
        recipeTitle,
        commonIssues,
      );
    } catch (error) {
  ...
  async getQuickTips(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get quick tips for recipe ${recipeId}:`,
        error,
      );
      return null;
    }
  ...
  async generateRecipeInsights(
    ...
    try {
      ...
      const insights = {
        ...
        quickSuggestions:
          quickSuggestions.status === 'fulfilled'
      ...
      this.logger.error(
        `Failed to generate recipe insights for ${recipeId}:`,
        error,
      );
  ...
  async generateDietaryAlternatives(
    @Args('recipeId') recipeId: string,
    @Args('restrictions', { type: () => [DietaryRestriction] })
    ...
    try {
      const substitutions =
        await this.ingredientSubstitutionService.getSubstitutions(
          recipeId,
      ...
      return {
        ...
        message: substitutions
          ? 'Dietary alternatives generated successfully'
      ...
      this.logger.error(
        `Failed to generate dietary alternatives for ${recipeId}:`,
        error,
      );
